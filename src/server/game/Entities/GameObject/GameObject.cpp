...
void GameObject::Update(uint32 diff)
{
    ...
    switch (m_lootState)
    {
        ...
        case GO_JUST_DEACTIVATED:
        {
            ...
            {
                if (_respawnDelay <= 600 && GetGOData() && !GetGOInfo()->GetXpLevel() && !GetMap()->Instanceable()) // §Ü§Ó§Ö§ã§ä§à§Ó§í§Ö §¤§° §Ú §á§â§à§é§Ú§Û §ê§Ý§Ñ§Ü
                {
                    uint32 targetCount = GetPlayerFromArea(GetGOData()->areaId);
                    if (targetCount)
                    {
                        if (targetCount >= sWorld->getIntConfig(CONFIG_RESPAWN_FROM_PLAYER_COUNT))
                            _respawnDelay /= targetCount; // §Ô§â§å§Ò§í§Û §â§Ñ§ã§ã§é§Ö§ä, §Ü§à§ß§Ö§é§ß§à, §ß§à §Ý§å§é§ê§Ö §å§Ø..
