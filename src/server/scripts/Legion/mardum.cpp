...
// A：这是一个playScene关联的Script...
// 通过 `world.spell_scene`表来关联
// 关联的pkgId是1487，sceneId:1106
...
public:
	...
	{
		NPC_CONV = 705,
		SPELL_CUEILLIDANTH = 191667,		// Devastator Bombardment 召唤一颗陨石炸自己...
	};

	// 应该是当player进入AreaTrigger的时候触发的？

	bool OnTrigger(Player* player, SpellScene const* /*trigger*/, std::string type) override
	{
		// Q:这个type是定义在什么地方的？
		// A： 从客户端消息来的 CMSG_SCENE_TRIGGER_EVENT ...
		// 也就是说这个函数应该是出来 playScene的event的 ...
...
// GO-244898，这个GO好像是LegionCore自己定义的...
// Quest-40077 的任务GO，交互进行插旗
	...
	struct go_q40077AI : public GameObjectAI
	{
		...
		bool GossipHello(Player* player) override
		{
			...
			*/
			...
			{
				player->CastSpell(player, SCENE, true);	// 播放插旗动画场景..
...
public:
	...
	bool OnQuestAccept(Player* player, Creature* creature, Quest const* quest) override
	{
		if (quest->GetQuestId() == QUEST)
		{
			...
			if (!conversation->CreateConversation(sObjectMgr->GetGenerator<HighGuid::Conversation>()->Generate(), NPC_CONV, player, NULL, *player))
				...
				// 93011 -- Kayn Sunfury  为什么要召唤自己？
				// 召唤一个新的npc出来 

				// 当接了任务之后，player的相位发生变化，会看不见当前这些npc，因此需要召唤出一组新的..
	...
	struct npc_q40077AI : public ScriptedAI
	{
		...
		void IsSummonedBy(Unit* summoner) override
		{
			...
			// 使用Kayn的Clone 召唤一队临时的npc ...
			// 为什么Kayn移动的时候，他召唤的npc也跟着移动？
			// 其他的npc由脚本 `npc_q40077_1` 驱动..
...
// GO-241751	Legion Gateway Activator
// 
	...
	struct go_q40378AI : public GameObjectAI
	{
		...
		bool GossipHello(Player* player) override
		{
			...
			*/
			...
			player->CastSpell(player, SCENE, true);	// 播放动画，召唤灰舌部队 ...
...
//
// 使用Spell摧毁邪刃啥东西？ 喷洒器？ 浮在控制的绿色的梭状器械
// Q:Spell 是怎么触发cast的？
// A: 通过 SpellEvent 来触发的19
// Fel Spreader - 97142 ... 这不是一个GO，而是一个Creature数据 ...
// 这个是通过`npc_spellclick_spells`表来将npc和spell进行关联的 ...
...
// 101748 - Fel Geyser			邪能喷泉
// 101781 - Spider Scene Stalker Controller		蜘蛛控制器 ..

// 进行喊话广播？ 绑定在什么npc上吗
// 好像是绑定在怪物上的...
// 当player满足条件的时候，播放Conversation..
...
public:
	...
	enum data
	{
		...
		SPELL_1 = 194519,  // 2 + 5				// Mind Spike
		SPELL_2 = 195058,  // 5					// Beaming Gaze
		SPELL_PHASE = 192665, // 53%				// Legion Aegis
		SPELL_INF = 192709,						// Infernal Smash
	};
...
// 这个脚本为了献祭灰舌法师来开启激活器，和npc对话后，npc献祭自己
...
public:
	...
	{
		QUEST = 40379,
		SPELL = 196724,		// Sacrificing Mystic's Soul
	};
...
public:
	...
	struct go_q40379AI : public GameObjectAI
	{
		...
		{
			QUEST = 40379,
			SPELL = 190793,			// Enter the Illidari: Coilskar - Play Scene
			CREDIT = 94406,
...
public:
	...
	struct go_q39050AI : public GameObjectAI
	{
		...
		{
			QUEST = 39050,
			SPELL = 188539,	// Meeting With the Queen: Ritual Complete
			CREDIT = 100722,
		};
...
public:
	...
	enum Credit
	{
		QUEST = 38765,
		SPELL = 196724,			// Sacrificing Mystic's Soul
		SPELL_1 = 196731,		// Sacrificing Sevis Brightflame
	};
...
public:
	...
	struct go_q38765AI : public GameObjectAI
	{
		...
		{
			QUEST = 38765,
			SPELL = 190851,	// Enter the Illidari: Shivarra - Play Scene
			CREDIT = 94407,
		};
	...
	struct npc_q40379_1AI : public ScriptedAI
	{
		...
		void IsSummonedBy(Unit* summoner) override
		{
			if (summoner->GetTypeId() == TYPEID_PLAYER)
			{
				...
				me->Mount(64385);
				me->GetMotionMaster()->MovePath(10267113, false);	// 路径
				me->DespawnOrUnsummon(10000);
			}
			else
				me->DespawnOrUnsummon();
